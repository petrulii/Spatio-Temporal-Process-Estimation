function [x, y, i] = mirror_prox(A, d, x_init, y_init, rate, index, max_iterations)
    ...
    while True
        ...
        if i >= max_iterations
            ...
            x_sample, y_sample = np.random.uniform(d), np.random.uniform(d);
...
function [rate] = linesearch_stepsize(A, x_i, y_i, x_i_1, grad_y_i, x2_i, y2_i, x2_i_1, grad_y2_i, rate)
...
function [gamma] = gradient_descent(A, x_init, y_init, figure = None, rate = 0.01, index = 0, iterations = 1000, gradient = matrix_game_gradient):
