...
function [theta, theta0, y, obj] = mirror_prox(N, L, d, time_series, kappa, max_iterations, true_theta, true_theta0, lambda)
    ...
    while i <= max_iterations
        ...
        % Calculate x_i+1.
        %theta = theta + prox(N, L, d, time_series, theta, theta0, rate, y, lambda);
...
% Prox mapping.
function [x] = prox(N, L, d, time_series, theta, theta0, rate, y, lambda)
    theta_vec = reshape(theta.',1,[]);
    cvx_begin
        variable x(L, d*L);
        x_vec = reshape(x.',1,[]);
        x_grad = gradient_theta(N, L, d, time_series, x, theta0, y, lambda);
        x_grad_vec = reshape(x_grad.',1,[]);
        minimize(dot(rate*x_grad_vec, x_vec-theta_vec) + norm(x_vec-theta_vec));
    cvx_end
end
